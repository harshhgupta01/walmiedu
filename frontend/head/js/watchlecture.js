...
function loadHeader()
{
    fetch(apiUrl+'/head/viewLecture/'+lectureId ,{
        method: 'GET',
        headers: 
        {
...
function setHeaderData(data)
{
    ...
    document.getElementById("header_lecture").innerText = data.result1.lecture_topic;
    document.getElementById("vimeo_video").src = data.result1.lecture_path;
    
    
    var lecture = document.getElementById("watch-list");
    if(!data.result3.length)
    {
        lecture.innerHTML="<h4 style='color:red'>No students have watched the lecture yet!</h4>";
    }
    else
    {
        for(i=0;i<data.result3.length;i++)
        {
            lecture.innerHTML += `
                <div class="card">
                    <p>${data.result3[i].student.student_name}</p>
                    <p>${data.result3[i].student.student_email}</p>
                </div>
            `;
        }
    }
